...
const MobileNavBar = ({ state, mouse }: States) => {
  ...
  return (
    <nav
      className="flex flex-col justify-between fixed inset-0 top-[3.8rem] left-[max(0px,calc(50%-45rem))] w-72 h-screen z-[200] px-2 pt-7 bg-[--background-color] dark:bg-[#111111]"
    ...
    >
      <div className="h-[88%]">
        ...
        <div className="h-[80%] mt-5">
          <span className="font-bold dark:text-[#DCDCDC]">Troybin properties</span>
          <hr className="border-gray-500 dark:border-gray-400 mt-1" />
          <ul className="flex flex-col items-start h-[92%] overflow-y-auto overflow-x-hidden dark:text-[#DCDCDC]">
        ...
        <div className="flex flex-row justify-center gap-10 border-t border-t-gray-500 dark:border-t-gray-200 py-3">
          ...
          >
            ...
            <IoSettings
              aria-label="settings"
              role="button"
              className="button text-3xl dark:text-[#DCDCDC] hover:animate-settings"
            />
          </div>
        </div>
      </div>
